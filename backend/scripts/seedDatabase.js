...
const runSeed = async () => {
  ...
  try {
    await session.withTransaction(async () => {
      await OtpToken.deleteMany({}, { session });
      await Diet.deleteMany({}, { session });
      await DietPlan.deleteMany({}, { session });
      await WorkoutRoutine.deleteMany({}, { session });
      await Workout.deleteMany({}, { session });
      await Session.deleteMany({}, { session });
      await Subscription.deleteMany({}, { session });
      await CoachProfile.deleteMany({}, { session });
      await TraineeProfile.deleteMany({}, { session });
